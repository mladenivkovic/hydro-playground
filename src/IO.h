...
public:
  ...
  void readParamFile(Parameters& params);

  //! Read the initial conditions file.
  void readICFile(Grid& grid);
...
private:
  ...
  static PrimitiveState _extractArbitraryICVal(std::string& line, size_t linenr);

  //! Read an IC file with the Two-State format
  void _readTwoStateIC(Grid& grid);

  //! Read an IC file with the arbitrary format
  void _readArbitraryIC(Grid& grid);
...
public:
  explicit OutputWriter(Parameters& params, Grid& grid):
    _noutputs_written(0),
    _t_last_dump(0.),
    _t_next_dump(0.),
    _params(params),
