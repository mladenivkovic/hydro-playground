...
class RiemannHLLC: public RiemannBase {
  ...
  __host__ __device__ Float _qLR(Float pstar, Float pLR);

  //! Compute the wave speeds SL, SR, Sstar
  __host__ __device__ void computeWaveSpeedEstimates();

  //! Compute the star states.
  __host__ __device__ void computeStarCStates(ConservedState& UStarL, ConservedState& UStarR);

  //! Sample the solution.
  __host__ __device__ ConservedFlux sampleHLLCSolution();
 ...
 */
inline void RiemannHLLC::computeWaveSpeedEstimates() {
  ...
  if (qmax <= 2. and (pmin <= PPV and PPV <= pmax)) {
    ...
    if (PPV <= pmin) {
      ...
      Float p1 = pR * std::pow((1. + aRinv * cst::GM1HALF * (vstar - vR)), cst::ONEOVERBETA);
      Float p2 = pL * std::pow((1. + aLinv * cst::GM1HALF * (vL - vstar)), cst::ONEOVERBETA);
      pstar    = 0.5 * (p1 + p2);
    }
 ...
 */
inline void RiemannHLLC::computeStarCStates(ConservedState& UStarL, ConservedState& UStarR) {

#if DEBUG_LEVEL > 0 and !__CUDA_ARCH__
 ...
 */
inline ConservedFlux RiemannHLLC::sampleHLLCSolution() {
#if DEBUG_LEVEL > 0 and !__CUDA_ARCH__
  if (_SL == 0. and _SR == 0. and _Sstar == 0.)
    warning("Suspicious wave speed estimates.");
#endif
